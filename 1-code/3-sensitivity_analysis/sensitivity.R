...
# Define the variables and sites to include in the heatmap
# average leaf area
...
# Define the variables and sites to include in the heatmap for bunch weight
var_check_bw <- c("average_bunch_weight_3_to_6", "average_bunch_weight_6_to_9", "average_bunch_weight_9_to_12")
short_labels_bw <- c("age 3 - 6", "age 6 - 9", "age 9 - 12")
names(short_labels_bw) <- var_check_bw

site_check_bw <- c("presco", "smse", "towe")
df_all_bw <- data.frame()

# Normalize mu_star and compute ranks for bunch weight
for (v in var_check_bw) {
    for (s in site_check_bw) {
        morris_result_bw <- all_res %>%
            filter(var == v, Site == s) %>%
            select(parameter, category, mu_star) %>%
            mutate(
                composite_index = (mu_star - min(mu_star, na.rm = TRUE)) /
        ...
        df_all_bw <- bind_rows(df_all_bw, morris_result_bw)
    }
}

# Extract top 10 per site and variable for bunch weight
df_10_bw <- df_all_bw %>%
    group_by(site, var) %>%
    slice_max(order_by = composite_index, n = 10, with_ties = FALSE) %>%
    ungroup()
...
df_10_bw <- df_10_bw %>%
    mutate(var_label = short_labels_bw[var])

df_10_bw$var_label <- factor(df_10_bw$var_label, levels = short_labels_bw)

# Plot for bunch weight
map_all_bw <- ggplot(df_10_bw, aes(x = var_label, y = parameter, fill = composite_index)) +
    geom_tile(color = "white", width = 1.0, height = 0.4) +
    geom_text(aes(label = round(composite_index, 2)), size = 3) +
    scale_fill_gradient2(
        low = "blue", mid = "white", high = "darkred",
...
# Save the plot for bunch weight
ggsave(
    # filename = "2-results/sensitivity/heatmap_average_bunch_weight.png",
    filename = "2-results/sensitivity/heatmap_average_bunch_weight_replace.png",
    plot = map_all_bw,
    bg = "white",
    width = 15,
    height = 6,
